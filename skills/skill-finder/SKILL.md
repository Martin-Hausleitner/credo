...
## Stack Integration (OpenClaw / Hermes)

This skill is part of the Matrix + Hermes Agent Stack. After installing a new skill, always update the inventory:

**Inventory file:** `docs/hermes-skills.md` in the repository  
`https://github.com/Martin-Hausleitner/matrix-hermes-agent-stack` (branch: `codex/stack-readme-inventory-map`)

**Local skills path:** `/Users/mh/.openclaw/workspace/skills/`

### Post-Install Checklist

After installing a skill with `clawhub install` or `skills add`:

1. Identify the correct category in `docs/hermes-skills.md`
2. Add the skill slug to the category row (alphabetically sorted)
3. If no category fits, add a new row
4. Add a row to **Besonders relevante Skills** if the skill is critical to the stack
5. Commit and push to the branch so the inventory stays current

```bash
# Check what's installed vs what's in the inventory
ls /Users/mh/.openclaw/workspace/skills/

# Update inventory, then push
cd /path/to/matrix-hermes-agent-stack
git add docs/hermes-skills.md
git commit -m "docs: add <skill-slug> to hermes-skills inventory"
git push
```

**Why:** The inventory (`hermes-skills.md`) is the single source of truth for which skills are available in the stack. Without it, agents and operators cannot discover installed capabilities.
